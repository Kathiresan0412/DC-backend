...
const businessDetailsFor = (business) => {
...
const getAgreementLink = (invoiceId) => `${getBaseUrl()}/agreements/${encodeURIComponent(invoiceId)}`;
...
const invoicePaymentLines = async (invoice) => {
    const payments = await paymentsCollection();
    const invoicePayments = await payments
        .find({ invoice_id: invoice.invoice_id })
        .sort({ paid_at: 1, created_at: 1 })
...
const invoiceEmailText = (invoice, paymentLines, agreementLink) => {
    const paymentHistory = paymentLines.length
        ? paymentLines.map((payment, index) => (`${index + 1}. ${payment.label} - ${payment.date} - ${payment.method} - ${formatMoney(payment.amount)} - Balance after: ${formatMoney(payment.receivableAfterPayment)}`)).join('\n')
...
const invoiceEmailHtml = (invoice, service, paymentLines, agreementLink) => {
